import { ContactInfo, ContactForm, ContactFooter } from "./components/index.js";
import { useFadeIn } from "../../hooks/useFadeIn.js";

function Contact() {
  const ref = useFadeIn();
  return (
    <section
      id="contact"
      className="flex flex-col min-h-svh section-gradient text-white"
    >
      <div className="flex-1 flex items-center justify-center py-24 max-md:py-16">
        <div
          ref={ref}
          className="fade-in w-full max-w-page mx-auto px-6 flex flex-col gap-12 max-md:gap-10"
        >
          <div className="flex flex-col items-start gap-5">
            <div className="flex items-center gap-3">
              <span className="text-livia-mustard text-lg" aria-hidden="true">
                ✦
              </span>
              <p className="text-eyebrow">Vamos conversar</p>
            </div>

            <h2 className="font-serif font-bold leading-[1.05] text-white text-[clamp(2.25rem,4vw,3.5rem)]">
              Vamos{" "}
              <em className="not-italic hero-color-cycle">trabalhar</em> juntos?
            </h2>
          </div>

          <div className="grid grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] gap-20 items-start max-lg:grid-cols-1 max-lg:gap-10">
            <ContactInfo />
            <ContactForm />
          </div>
        </div>
      </div>

      <div className="w-full max-w-page mx-auto px-6 pb-8">
        <ContactFooter />
      </div>
    </section>
  );
}

export default Contact;
